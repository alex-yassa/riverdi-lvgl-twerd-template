...
#else
static __attribute__((aligned(32))) uint8_t buf_1[MY_DISP_HOR_RES * 15 * 2];
static __attribute__((aligned(32))) uint8_t buf_2[MY_DISP_HOR_RES * 15 * 2];
#endif
            ...
            uint8_t * px_map)
{
  ...
  DMA2D->CR = 0x0U << DMA2D_CR_MODE_Pos;	// no conversion, M2M
  ...
  DMA2D->FGMAR = (uint32_t)px_map;
  DMA2D->FGOR = 0;  /* source is tightly packed: width pixels per row, no gap */
#endif
