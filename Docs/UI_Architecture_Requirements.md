...
|   | - Initial LTDC Power-On  |     | - Touch Screen Driver    |   |
...
* **HMI Input Processing:** Directly manages the I2C Touch Screen controller bus and runs the UI tick timing tracking (`lv_tick_inc`).
...
3. **Peripheral Migration:** Shift the I2C configuration for the touch panel and the hardware timer instance tracking systemic system ticks into the M4 core initialization steps.
